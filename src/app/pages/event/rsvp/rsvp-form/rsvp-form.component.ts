...
import { GUESTS_REGEX } from '../../../../core/forms/form-utils.factory';

interface RsvpEventObj {
  isEdit: boolean;
  rsvp?: RsvpModel;
  error?: any;
}
...
export class RsvpFormComponent implements OnInit, OnDestroy {
  ...
  @Output() submitRsvp = new EventEmitter<RsvpEventObj>();
  ...
  private _handleSubmitSuccess(res: RsvpModel) {
    const eventObj: RsvpEventObj = {
      isEdit: this.isEdit,
      rsvp: res
    };
  ...
  private _handleSubmitError(err: any) {
    const eventObj: RsvpEventObj = {
      isEdit: this.isEdit,
      error: err
    };
